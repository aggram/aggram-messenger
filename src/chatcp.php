...
.msg-actions {
    position: absolute;
    top: 0;
    right: 100%;          /* منو سمت راست پیام باز شود */
    margin-right: 50px;
    margin-left: 8px;    /* کمی فاصله از پیام */
    background: #182533; /* رن تیره شبیه تلگرام */
    border-radius: 12px;
    padding: 6px 0;
    z-index: 1000;
    min-width: 170px;
...
.menu-item {
    display: flex;
    justify-content: space-between;  /* متن چپ، آیکون راست */
    align-items: center;
    padding: 6px 14px;
    color: #fff;
    cursor: pointer;
    ...
    white-space: nowrap;            /* متن رو در یک خط نگه می‌دارد */
}
    ...
    <div class="chat-header">
        ...
        <div id="chat-header-select" style="display: none;">
            ...
            <div class="select-right">
                ...
                <button class="icon-btn" id="btn-select-reply"    title="Reply">↩ Reply</button>
                <button class="icon-btn" id="btn-select-pin"      title="Pin">📌 Pin</button>
                <button class="icon-btn" id="btn-select-copy"     title="Copy">📋Copy</button>
                <button class="icon-btn" id="btn-select-forward"  title="Forward">➡ Forward</button>
                <button class="icon-btn" id="btn-select-save"     title="Save">💾 Save message</button>
                <button class="icon-btn" id="btn-select-edit"     title="Edit">✏️ Edit</button>
                <button class="icon-btn" id="btn-select-delete"   title="Delete">🗑Delete</button>
            </div>
        </div>
    </div>
...
function addMessage(id, uuid, text, isSelf, time, isSeen, messageId) {
    const tick = isSelf ? (isSeen ? "✓✓" : "✓") : "";
    const div = document.createElement("div");
    div.className = "message " + (isSelf ? "self" : "other");
